...
- Mass is ground-truth from the manual weigh table (weights.csv), NOT load cells.
...
| Cone analysis | P01 REVISED + weights.csv | **DONE** (CONE_FINDINGS.md, cone_loader.py, cone_analysis.py, figures/cone_*, results/cone_*) |
| Exponat recon | (folded into RECON §B.4) | DONE |
| Exponat analysis | P02 | **DONE** (EXPONAT_FINDINGS.md, exponat_loader.py, exponat_analysis.py, figures/exponat_*, results/exponat_*) |
...
| P1 T3 wall-heating | — | **RUNNING** job 21899510 `m2_base_nest20_450` (2mm→450s, 48h, DT_RESTART=7200). SLOW — load-imbalanced (quiet plenum meshes stall on fire mesh), ~0.9 s/step, ~48h projected → will need `RESTART=.TRUE.` resume. ~47s sim at 4h. |
| P2 T3 wall-attribution | — | **RUNNING** jobs 21900243 `s5_wallins` / 21900244 `s6_walladi` — **8-rank** `--split 4,1,2`, T_END 350s, 18h. (First attempt 21899511/512 was 2-rank/8-core → 1.9 s/step, cancelled + resplit.) Clean divergence, ~1 min/simsec → ~6h. **These carry most of the T3 decomposition; post-process first when they land.** |
| P3 source-uncertainty sweep | — | **DECKS READY + PARSE-VERIFIED — fds/sweep/** s0-s4 (HRR 15/21, χr 0.20/0.35), 5mm/8-rank/350s. Submit after P2 (`submit_sweep.sh 2`; user's script now also submits s7_tracer there). |
| sensitivity_post.py | — | **DONE + TESTED** (synthetic). results/sensitivity_bands.csv + sensitivity_tables.md: per-sensor knob bands, itemised model-discrepancy table, T3 decomposition. `SWEEP_RUNS_DIR` env override. Degrades gracefully with only s0/s5/s6. |
| 7-TC validation figure | — | **DONE — figures/validation_7TC_grid.png** (fig_validation_grid.py). Approved for the report. |
| P4 smoke | — | **METRIC = option (a): passive tracer, transport shape+timing only, never concentration.** `--tracer` added to make_fds.py (fog from cup SIDE faces via `&SURF FOG_SRC` — NOT the burner: species MASS_FLUX + HRRPUA on one surf → FDS scales flux off HRR, 1.5e10 kg/s/m2 bug, both indexed & unindexed). s7_tracer deck 5mm/8-rank/350s, parse-check pc7c pending. **SMOKE_FINDINGS.md written** (preliminary): real result = NO sharp interface, gradual fill over minutes, corroborates P02. **fog_digitize.py built** — calib anchored on room floor/ceiling; ignition 3276@~120s / 3278@~30s; x=0.35 scan occlusion-limited. Video at /Volumes/room_corner/Samia/ (7L5A3276=fog exp1, 3277/3278=fog exp2 [78 dark], 25fps). Don't build tracer↔fog comparison until s7_tracer done AND digitization validated. |
| multi-session | — | 2026-09-08: peers firescope-f3 (repo launcher + main CLAUDE.md scrub) and firescope-68 (kids repo) active — confirmed NO collision with experiments/. make_fds.py / sweep-script edits seen on disk = user direct. This session (firescope-be) owns cluster + make_fds.py + fds/sweep/*. |
