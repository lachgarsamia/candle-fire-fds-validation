...
| src/sensitivity_post.py | — | **DONE + TESTED** (synthetic). data/processed/sensitivity_bands.csv + sensitivity_tables.md: per-sensor knob bands, itemised model-discrepancy table, T3 decomposition. `SWEEP_RUNS_DIR` env override. Degrades gracefully with only s0/s5/s6. |
| 7-TC validation figure | — | **DONE — figures/validation_7TC_grid.png** (src/fig_validation_grid.py). Approved for the report. |
| P4 smoke | — | **METRIC = option (a): passive tracer, transport shape+timing only, never concentration.** `--tracer` added to make_fds.py (fog from cup SIDE faces via `&SURF FOG_SRC` — NOT the burner: species MASS_FLUX + HRRPUA on one surf → FDS scales flux off HRR, 1.5e10 kg/s/m2 bug, both indexed & unindexed). s7_tracer deck 5mm/8-rank/350s, parse-check pc7c pending. **SMOKE_FINDINGS.md written** (preliminary): real result = NO sharp interface, gradual fill over minutes, corroborates P02. **src/fog_digitize.py built** — calib anchored on room floor/ceiling; ignition 3276@~120s / 3278@~30s; x=0.35 scan occlusion-limited. Video at /Volumes/room_corner/Samia/ (7L5A3276=fog exp1, 3277/3278=fog exp2 [78 dark], 25fps). Don't build tracer↔fog comparison until s7_tracer done AND digitization validated. |
